...
public final class Knob1 extends RangeControl {
	 ...
	 */
	public Knob1(float x, float y, float width, float height) {
	...
	protected void render() {
		...
		if (canDrag) {
			getInternalValue().append(ctx.pmouseY - ctx.mouseY);
		}
	...
	private void indicatorOnDraw() {
		...
		ctx.arc(0, 0, diameter * .8f, diameter * .8f, START, MathUtils.convert(getInternalValue().get(),
				getInternalValue().getMin(), getInternalValue().getMax(), START, END));
