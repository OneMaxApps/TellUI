...
/**
 * Advanced event handler that manages event listeners and dispatches events to registered callbacks.
 * Extends AbstractEventSystem to provide a comprehensive event handling system with
 * listener registration and event dispatching capabilities.
 * <p>
 * The InteractionHandler not only detects mouse events but also manages listener registration
...
public final class InteractionHandler extends AbstractEventSystem {
	...
	/**
	 * Updates event detection and dispatches events to registered listeners.
	 * This method should be called every frame for active event handling.
	 */
	@Override
	public void listen() {
		if(!isEnabled()) {
	...
	/**
	 * Enables or disables this interaction handler.
	 * When disabled, no events will be detected or dispatched.
	 * 
	 ...
	 * @throws NullPointerException if eventType or listener is null
	 * @throws IllegalStateException if the listener is not found for the given event type
	 */
	...
	private final class EventDispatcher {
		...
		/**
		 * Safely adds a listener for a specific event type.
		 * Creates the listener list if it doesn't exist.
		 * 
