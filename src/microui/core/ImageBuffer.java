...
/**
 * A spatial view component for displaying and manipulating images with color tinting.
 * ImageBuffer provides functionality for loading, displaying, and applying color tints
 * to PImage objects, with automatic dimension clamping to prevent oversized images.
 * 
...
public class ImageBuffer extends SpatialView {
     ...
     * The buffer is initially visible with white tint color and no loaded image.
     */
    public ImageBuffer() {
        setVisible(true);
        color = Color.WHITE;
    ...
    /**
     * Draws the image buffer if an image is loaded.
     * Overrides the parent draw method to check for loaded image before rendering.
     */
    @Override
    public void draw() {
        if (isLoaded()) {
    ...
    /**
     * Sets the image directly from a PImage object.
     * Clamps the image dimensions to prevent oversized images.
     *
     ...
     * The tint color is multiplied with the image colors during rendering.
     *
    ...
    /**
     * Removes the currently loaded image texture.
     * After calling this, isLoaded() will return false until a new image is set.
     */
