...
public abstract class AbstractEventSystem {
	...
	protected final class EventDetector {
		...
		public boolean isPressed() {
			return hover && TellUI.getContext().mousePressed;
		}
		...
		public boolean isHover() {
			...
			int mx = TellUI.getContext().mouseX;
			int my = TellUI.getContext().mouseY;
		...
		private final class PressDetector {
			...
			public boolean isDetected() {
				...
				if (!TellUI.getContext().mousePressed) {
		...
		private final class ReleaseDetector {
			...
			public boolean isDetected() {
				...
				if (!TellUI.getContext().mousePressed && !isReleaseHookCalled) {
					isReleaseHookCalled = true;
					releaseTime = System.currentTimeMillis();
		...
		private final class DragStartDetector {
			...
			private boolean isMouseMoved() {
				int mx = TellUI.getContext().mouseX;
				int my = TellUI.getContext().mouseY;
				int pmx = TellUI.getContext().pmouseX;
				int pmy = TellUI.getContext().pmouseY;
		...
		private final class DragEndDetector {
			...
			public boolean isDetected() {
				...
				if (!dragStartDetector.isDetected() && isDragEnd && !TellUI.getContext().mousePressed) {
					isDragEnd = false;
